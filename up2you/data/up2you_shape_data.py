...
def read_shape_from_pkl(pkl_path):
    params = pkl.load(open(pkl_path, "rb"))
    shape = params["betas"].reshape(1, 10).float()
...
def _parse_one_json_scene_train(json_path: str, num_refs: int, num_views: int=6):
    ...
    for id_data in json_data:
        ...
        # Read shape params from the first case's smplx_params
        first_case = id_data["cases_target_data"][0]
        shape_param_path = first_case["target_smplx_params_path"]
        for case_data in id_data["cases_target_data"]:
            scene = {}
            scene["ref_image_paths"] = ref_image_paths.copy()
            scene["shape_param_path"] = shape_param_path
...
class MVPuzzleDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.used_data_type is not None and "target_shapes" in self.used_data_type:
            target_shapes = read_shape_from_pkl(scene_dir["shape_param_path"])
            out_dir["target_shapes"] = target_shapes
    ...
    def collate(self, batch):
        ...
        for k in batch.keys():
            if k in ["ref_rgbs", "ref_alphas", "target_shapes"]:
...
class MultiviewPuzzleDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = MVPuzzleDataset(self.cfg, 'train')
