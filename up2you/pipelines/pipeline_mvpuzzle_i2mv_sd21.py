...
from diffusers.utils import deprecate, is_torch_xla_available, logging, BaseOutput
...
class UP2YouI2MVSDPipeline(StableDiffusionPipeline, CustomAdapterMixin):
    def __init__(
        self,
        vae: AutoencoderKL,
    ...
    def __call__(
        ...
        # Image condition
        reference_rgbs: Optional[PipelineImageInput] = None,
        reference_conditioning_scale: Optional[float] = 1.0,
        weight_maps: Optional[List[torch.Tensor]] = None,
        ref_img_masks: Optional[torch.Tensor] = None,
        # Feature selector
        ...
        **kwargs,
        ...
        reference_image = self.image_processor.preprocess(reference_rgbs)

        num_ref = reference_image.shape[0] // batch_size
        ...
        ref_timesteps = torch.zeros_like(timesteps[:1]).repeat_interleave(num_ref, dim=0)
        # print("*"*100)
        ...
        with torch.no_grad():
            self.unet(
                reference_latents,
                ref_timesteps,
                encoder_hidden_states=prompt_embeds[-1:].repeat_interleave(num_ref, dim=0),
            ...
            for k, v in ref_hidden_states.items():
                _, N, C = v.shape
                h_ref = w_ref = int(N ** 0.5)
                v_ = rearrange(v, "(B Nr) (H W) C-> B Nr H W C", B=batch_size, Nr=num_ref, H=h_ref, W=w_ref)
                v_ = self.feature_selector.forward(ref_img_feats=v_, weight_maps=weight_maps, ref_img_masks=ref_img_masks, max_select_ratio=max_select_ratio, random_select_ratio=random_select_ratio)
                ref_hidden_states[k] = v_ # no need to repeat_interleave
    ...
    def _init_custom_adapter(
        ...
        if copy_attn_weights:
            ...
            for key in state_dict.keys():
                if "_mv" in key:
                    ...
                    compatible_key = key.replace("_global", "").replace("processor.", "")
                else:
                    compatible_key = key
        ...
        self.feature_selector = FeatureSelector(mode=mode)
