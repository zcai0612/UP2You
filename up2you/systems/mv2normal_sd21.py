...
from ..pipelines.pipeline_mvpuzzle_mv2normal_sd21 import MVPuzzleMV2NormalPipeline
...
class MVPuzzleMV2NormalSystem(BaseSystem):
    @dataclass
    ...
    def configure(self):
        ...
        pipeline: MVPuzzleMV2NormalPipeline
        ...
        pipeline = MVPuzzleMV2NormalPipeline.from_pretrained(
        ...
        # Prepare models
        self.pipeline: MVPuzzleMV2NormalPipeline = pipeline
    ...
    def forward(
        ...
        conditioning_pixel_values: Tensor, # target pose
        timesteps: Tensor,
        ref_latents: Tensor,
        prompts: List[str],
        num_views: int,
        ...
        for k, v in ref_hidden_states.items():
            v[image_drop_mask_ref] = 0.0
            ref_hidden_states[k] = v
    ...
    def training_step(self, batch, batch_idx):
        ...
        target_normals = batch["target_normals_camera"]
        target_poses = batch["target_poses_camera"]

        vae_max_slice = 8
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=False):
            latents = []
            for i in range(0, target_normals.shape[0], vae_max_slice):
                latents.append(
                    vae_encode(
                        self.vae,
                        target_normals[i : i + vae_max_slice].to(self.vae.dtype) * 2 - 1,
                        sample=True,
                        apply_scale=True,
                    ).float()
                )
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(enabled=False):
            ref_latents = []
            for i in range(0, target_rgbs.shape[0], ref_vae_max_slice):
                ref_latents.append(
                    vae_encode(
                        self.vae,
                        target_rgbs[i : i + ref_vae_max_slice].to(self.vae.dtype) * 2 - 1,
                        sample=True,
                        apply_scale=True,
                    ).float()
                )
        ...
        self.log("loss_sd", loss_sd, prog_bar=True, logger=True)
    ...
    def get_input_visualizations(self, batch):
        return [
            ...
            {
                "type": "rgb",
                "img": rearrange(batch["target_rgbs"], "(B Nv) C H W -> (B H) (Nv W) C", Nv=batch["num_views"]),
                "kwargs": {"data_format": "HWC"},
            },
            {
                "type": "rgb",
                "img": rearrange(
                    batch["target_normals_camera"], "(B Nv) C H W -> (B H) (Nv W) C", Nv=batch["num_views"]
                ),
                "kwargs": {"data_format": "HWC"},
            },
        ]
    ...
    def get_output_visualizations(self, batch, outputs):
        images = [
            ...
            },
            {
                "type": "rgb",
                "img": rearrange(
                    outputs, "(B Nv) C H W -> (B H) (Nv W) C", Nv=batch["num_views"]
                ),
                "kwargs": {"data_format": "HWC"},
            },
    ...
    def generate_images(self, batch, **kwargs):    
        return self.pipeline(
            ...
            reference_rgbs=batch["target_rgbs"],
            output_type="pt",
        ).images
    ...
    def validation_step(self, batch, batch_idx):
        images = self.generate_images(batch)
        ...
        if (
            ...
            self.save_image_grid(
                ...
                self.get_output_visualizations(batch, images),
