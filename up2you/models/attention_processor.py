...
class DecoupledMVRowSelfAttnProcessor_feats(torch.nn.Module):
    r"""
    ...
    def __init__(
        ...
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError(
                "DecoupledMVRowSelfAttnProcessor_feats requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0."
            )
        ...
        if self.use_ref:
            ...
            self.to_k_ref = nn.Linear(
                in_features=ref_dim, out_features=inner_dim, bias=False
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if use_ref:
            key_ref = self.to_k_ref(ref_hidden_states)
            value_ref = self.to_v_ref(ref_hidden_states)
