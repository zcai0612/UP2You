...
class CrossAttnBlock(nn.Module):
    ...
    def forward(self, x: Tensor, context: Tensor, q_pos=None, k_pos=None) -> Tensor:
        def attn_residual_func(x: Tensor, context: Tensor, q_pos=None, k_pos=None) -> Tensor:
            return self.ls1(self.attn(self.norm1(x), context=context, q_pos=q_pos, k_pos=k_pos))
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            x = x + self.drop_path1(attn_residual_func(x, context=context, q_pos=q_pos, k_pos=k_pos))
            x = x + self.drop_path1(ffn_residual_func(x))  # FIXME: drop_path2
        else:
            x = x + attn_residual_func(x, context=context, q_pos=q_pos, k_pos=k_pos)
